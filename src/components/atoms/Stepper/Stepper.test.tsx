...
describe('Stepper', () => {
  ...
  it('applies complete connector styling for completed steps', () => {
    ...
    const connectors = screen.getAllByText('Step 1')[0].closest('div')?.querySelectorAll('.connector');
  ...
  it('applies custom className', () => {
    ...
    expect(screen.getByText('Step 1').closest('div')).toHaveClass('custom-class');
  });

  it('applies custom id', () => {
    render(<Stepper steps={mockSteps} id="test-id" />);
    
    expect(screen.getByText('Step 1').closest('div')).toHaveAttribute('id', 'test-id');
  });
  ...
  it('applies custom style', () => {
    ...
    expect(screen.getByText('Step 1').closest('div')).toHaveStyle({ backgroundColor: 'red' });
  });
  ...
  it('handles asterisk in step labels', () => {
    const stepsWithAsterisk = [
      { id: '1', content: '1', label: 'Required Step', status: 'default' as const, showAsterisk: true },
      { id: '2', content: '2', label: 'Optional Step', status: 'default' as const, showAsterisk: false },
    ];
  ...
  it('renders single step', () => {
    const singleStep = [{ id: '1', content: '1', label: 'Single Step', status: 'active' as const }];
